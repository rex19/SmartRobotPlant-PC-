...
export default class Sanding extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      SandingAlermRecordModalOpen: false,  //弧焊报警模态框
      SandingChartsModalOpen: false,  //弧焊图标模态框
  ...
  SandingCellStateModalClose = () => {
    clearInterval(AlarmRecordModalSettimeout), clearInterval(AjaxCellSerialNoModalSettimeout);
    this.props.CloseSandingCellStateModal();
  }

  //报警信息Modal 打开/Close/AJAX
  SandingAlermRecordModalShow = (size, CellNo) => () => {
    this.setState({ size, SandingAlermRecordModalOpen: true })
    this.AjaxCellAlermRecordModal(CellNo)
    AlarmRecordModalSettimeout = setInterval(() => this.AjaxCellAlermRecordModal(CellNo), 10000);
  }
  SandingAlermRecordModalClose = () => { this.setState({ SandingAlermRecordModalOpen: false }), clearInterval(AlarmRecordModalSettimeout); }
  ...
  SandingChartsModalShow = (size, CellNo) => () => {
    this.setState({ size, SandingChartsModalOpen: true });
    this.AjaxSandingChartsModal(CellNo);
    clearInterval(AlarmRecordModalSettimeout), clearInterval(AjaxCellSerialNoModalSettimeout);
    this.props.clearIntervalOther();
    AjaxCellSerialNoModalSettimeout = setInterval(() => this.AjaxSandingChartsModal(this.props.CellNo), SetIntervalTime);
    this.setState({ ButtonColor: '#EB3F2F', ButtonValue: 'Stop Real-time', ButtonIcon: 'pause' })
  }
  SandingChartsModalClose = () => {
    this.setState({ SandingChartsModalOpen: false })
    clearInterval(AjaxCellSerialNoModalSettimeout)
    this.props.OpenIntervalOther()
  }
  AjaxSandingChartsModal = async (CellNo) => {
    console.log('AjaxSandingChartsModal-timer4')
    try {
      let CRUDAjax = await crud.fanucKanbanAjaxSelectPolishUnitMeasurementHistoryByRobotID(CellNo);
      ...
      console.log('Cell.js/AjaxSandingChartsModal/Ajax:', error)
    }
  }
  ...
  render() {
    const { SandingAlermRecordModalOpen, SandingChartsModalOpen, NoDataModalOpen, size } = this.state
    return (
      <div>
        {/*CellState  Sanding Modal     */}
        <Modal size={'fullscreen'} open={this.props.SandingCellStateModalOpen} onClose={this.SandingCellStateModalClose} closeIcon='close' basic>
          <Modal.Header>
            <a style={{ color: 'red', fontSize: '2rem' }}>FANUC</a> &nbsp;&nbsp;Polish CellState
                     </Modal.Header>
          <Modal.Content>
            <Grid>
              <Grid.Row columns={4}>
                ...
                <Grid.Column>
                  <Button onClick={this.SandingAlermRecordModalShow('fullscreen', this.props.CellNo)} fluid>Robot AlarmRecord</Button>
                </Grid.Column>
                <Grid.Column>
                  <Button onClick={this.SandingChartsModalShow('fullscreen', this.props.CellNo)} fluid>Real-time ProcessData</Button>
                </Grid.Column>
                <Grid.Column>
                  <Button.Group fluid>
                    <Button>Yield</Button>
          ...
          <Modal.Actions>
            <Button negative onClick={this.SandingCellStateModalClose} >
              Close
                        </Button>
          </Modal.Actions>
        </Modal>

        {/*SandingAlermRecordModal*/}
        <Modal size={size} open={SandingAlermRecordModalOpen} onClose={this.SandingAlermRecordModalClose} closeIcon='close' basic>
          <Modal.Header >
          ...
          <Modal.Actions>
            <Button negative onClick={this.SandingAlermRecordModalClose}>
              Close
                        </Button>
          </Modal.Actions>
        </Modal>
        ...
        <Modal size={size} open={SandingChartsModalOpen} onClose={this.SandingChartsModalClose} closeIcon='close' basic>
          ...
          <Modal.Actions className='ModalContentClass'>
            <Button negative onClick={this.SandingChartsModalClose}>
              Close
                        </Button>
          </Modal.Actions>
        </Modal>
